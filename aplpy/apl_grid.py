...
from apl_ticks import tick_positions
import numpy as np
from matplotlib.collections import LineCollection
from apl_util import complete_range
...
def plot_grid(ax):
	...
	if ax.apl_show_grid:
		...
		lon_all = complete_range(0.0,360.0,xspacing)
		...
		if np.size(lon_ni) > 0:
			...
			xpix,ypix = wcs.wcs_sky2pix_fits(lon_ni,lat_ni)
	...
	#				plot_longitude(wcs,np.array([]),np.array([]),lon_ni[i])
		...
		lat_all = complete_range(-90.0,90.0,yspacing)
...
#			print "[grid] updating"
			...
			ax.collections[grid_id].set_color(ax.apl_grid_color_default)
			ax.collections[grid_id].set_alpha(ax.apl_grid_alpha_default)
		else:
#			print "[grid] creating"
			c = LineCollection(polygons_out,transOffset=ax.transData)
			c.grid = True
			c.set_color(ax.apl_grid_color_default)
			c.set_alpha(ax.apl_grid_alpha_default)
...
def plot_latitude(wcs,lon,lat,lat0,alpha=0.5):
	...
	xpix,ypix = wcs.wcs_sky2pix_fits(np.array([0.]),np.array([lat0]))
	...
	for l in lonnew:
		...
		if(inside):
			...
			x_pix,y_pix = wcs.wcs_sky2pix_fits(x_world,y_world)
...
def plot_longitude(wcs,lon,lat,lon0,alpha=0.5):
	...
	xpix,ypix = wcs.wcs_sky2pix_fits(np.array([10.]),np.array([-90.]))\
